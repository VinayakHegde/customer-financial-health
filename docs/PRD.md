...
| R20 | On every outcome screen, the product is clearly framed as a reflection of the customer's own numbers and not as financial advice. Wording is chosen so a reasonable customer would not mistake the output for a regulated advice service. | Should | Regulatory | brief lines 17–19 (FCA obligations / fair treatment / explainability), brief lines 69–70 (how decisions are surfaced to users); `NOTES.md` §1 "Why this is hard" (reflection layer must not behave like a collections workflow or nudge into unaffordable positions), §5 Regulatory (Consumer Duty paraphrase — non-misleading, fair-value framing). Pairs with R6 (tone) and R9 (explainability) — R20 commits to *what the product is and is not* in customer-visible framing, on top of R6's *how it speaks* and R9's *why the band was assigned*. |
...
| Reflection-not-advice framing — the product must not be mistaken for regulated financial advice (brief lines 17–19, 69–70; `NOTES.md` §1 "Why this is hard", §5 Regulatory) | R20 | Should; communicative-posture commitment that sits alongside R6 (tone) and R9 (explainability). R6 governs *how it speaks*; R9 governs *why the band was assigned*; R20 governs *what the product is and is not* in customer-visible framing. Added to close the F4.3 gate-cross surfaced by the tech-spec critic (S007). |
